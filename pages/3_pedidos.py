...
if not st.session_state.get('carrinho'):
    ...
    if not st.session_state.pedido_enviado:
        if st.button("🚀 SALVAR PEDIDO E PREPARAR WHATSAPP", type="primary"):
            with st.spinner("Registrando pedido na planilha..."):
                payload = {
                    "action": "create",
                    "nome": st.session_state.usuario.get('nome', 'Cliente'),
                ...
                try:
                    ...
                    if res.status_code == 200:
                        ...
                        st.error(f"Erro ao salvar (Status {res.status_code})")
                        st.write("Resposta:", res.text)
                except Exception as e:
                    st.error(f"Erro de conexão: {e}")
                    st.info("Verifique sua internet ou se o Apps Script está ativo.")
    else:
        # === PARTE DO WHATSAPP (agora visível) ===
        ...
        st.markdown(f"""
            <div style="text-align: center; background-color: #e8f5e9; padding: 25px; border-radius: 15px; border: 3px solid #25D366; margin: 20px 0;">
